...
| File | Responsibility |
|---|---|
| `js/checkouts.js` | `CHECKOUT_TABLE` (2–170), `getCheckoutHint()`, `getAllCheckoutHints()`, `DOUBLE_FINISHES` set, `isDoubleAttemptScore()`, `closeDoubleLabel(val)`, `findCheckoutPath(score, targetDouble, dartsLeft)`, `summaryDoubleAttemptCount(remaining, visitScore, isBust)` (helper for summary-mode double tracking — 3 branches: direct double → bust=1/normal=3; ≤50 needs setup → bust=1/normal=2; >50 and post-visit leaves direct double → 2; else 0) |
...
| 3b | **Groups format only** — number of groups (valid k where `3 ≤ floor(n/k) ≤ 8`); advance count per group (global input or per-group toggle with individual inputs, limit `groupSize−1`; minimum 2 when `n === 3 && numGroups === 1`); win/loss points. `_initStep3bGroupButtons()` builds group-count buttons dynamically. |
