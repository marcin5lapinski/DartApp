...
| 2 | Format (liga, bracket, or groups+bracket); rounds (single/double, liga only); win/loss points (liga only). Format settings share the same vertical space via CSS grid overlay. Bracket format shows `#bracket-settings` div with hint text + `#t-bracket-third-place-match` checkbox (replaces old `#t-bracket-desc`). |
